...
interface AiState {
  ...
  currentFrame: number;
  message: string;
  currentVideo: { readonly name: string; readonly size: number; readonly type: string; readonly sourceUrl: string } | null;
...
function revokeVideoUrl(video: AiState["currentVideo"]): void {
  if (video?.sourceUrl.startsWith("blob:")) URL.revokeObjectURL(video.sourceUrl);
}
...
export const useAiStore = create<AiState>((set, get) => ({
  ...
  begin: (file) => {
    revokeVideoUrl(get().currentVideo);
    set({ currentStep: "uploading", progress: 1, currentFrame: 0, message: "Uploading video to local AI service", currentVideo: { name: file.name, size: file.size, type: file.type, sourceUrl: URL.createObjectURL(file) }, metadata: null, result: null, error: null, jobId: null });
  },
  ...
  reset: () => {
    revokeVideoUrl(get().currentVideo);
    set({ currentStep: "idle", progress: 0, currentFrame: 0, message: "Choose a local video to begin.", currentVideo: null, metadata: null, result: null, error: null, jobId: null });
  }
}));
